...
if __name__ == '__main__':
    ...
    #print(size)#(854, 480)
    ## 调用VideoWrite（）函数
    #videoWrite = cv2.VideoWriter('MySaveVideo.avi', cv2.VideoWriter_fourcc('I', '4', '2', '0'), fps_num, size)
    ...
    while fvs.more():
        ...
        for res in boxes:
            # self.margin_clip(res)
            cv2.rectangle(img, (res[0], res[1]),(res[2], res[3]), (255, 255, 0),3)
        #videoWrite.write(img)
        fx = 0.5
        fy = 0.5
        img=cv2.resize(img, (0, 0), fx=fx, fy=fy, interpolation=cv2.INTER_CUBIC) 
        cv2.imwrite(os.path.join(result_dir,"{}.jpg".format(index)),img)
        index += 1
        #cv2.imshow('res', img)
        #cv2.waitKey(100)